...
public class ManagementAccount extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        for (Ordered ord : myOrders) {
            ...
            if (trans.getStatus().equals(OrderStatus.DOING)) {
                deliverings.add(ord);
            }
            if (trans.getStatus().equals(OrderStatus.DONE)) {
                delivereds.add(ord);
            }
            if (trans.getStatus().equals(OrderStatus.CANCEL)) {
                cancelleds.add(ord);
            }
            if(trans.getStatus().equals(OrderStatus.CONFIRM)){
                confirm.add(ord);
            }
        }
        
        for (Transactions transs : tran) {
            if (transs.getStatus().equals(OrderStatus.DOING)) {
                transDeliverings.add(transs);
            }
            if (transs.getStatus().equals(OrderStatus.DONE)) {
                transDelivereds.add(transs);
            }
            if (transs.getStatus().equals(OrderStatus.CANCEL)) {
                transCancelleds.add(transs);
            }
            if(transs.getStatus().equals(OrderStatus.CONFIRM)){
